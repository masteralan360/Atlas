CREATE TABLE IF NOT EXISTS public.workspace_branches (
  ...
  branch_workspace_id uuid NOT NULL REFERENCES public.workspaces(id) ON DELETE CASCADE,
  name text NOT NULL,
...
ALTER TABLE public.workspace_branches
  DROP CONSTRAINT IF EXISTS unique_branch_workspace;

ALTER TABLE public.workspace_branches
  ADD CONSTRAINT unique_branch_workspace UNIQUE (branch_workspace_id);

ALTER TABLE public.workspace_branches
  DROP CONSTRAINT IF EXISTS no_self_branch;

ALTER TABLE public.workspace_branches
  ADD CONSTRAINT no_self_branch CHECK (source_workspace_id <> branch_workspace_id);

CREATE INDEX IF NOT EXISTS idx_workspace_branches_source_workspace
  ON public.workspace_branches (source_workspace_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_workspace_branches_branch_workspace
  ON public.workspace_branches (branch_workspace_id);
...
CREATE POLICY workspace_branches_insert
  ON public.workspace_branches
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY workspace_branches_delete
  ON public.workspace_branches
  FOR DELETE
  TO authenticated
  USING (
...
BEGIN
  ...
  SET workspace_id = p_source_workspace_id
