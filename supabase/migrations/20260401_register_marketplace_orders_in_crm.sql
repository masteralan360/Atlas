ALTER TABLE public.workspaces
  ADD COLUMN IF NOT EXISTS ecommerce_auto_create_customers boolean;

UPDATE public.workspaces
SET ecommerce_auto_create_customers = COALESCE(ecommerce_auto_create_customers, false);

ALTER TABLE public.workspaces
  ALTER COLUMN ecommerce_auto_create_customers SET DEFAULT false;

ALTER TABLE public.workspaces
  ALTER COLUMN ecommerce_auto_create_customers SET NOT NULL;
...
BEGIN
  ...
  NEW.ecommerce_auto_create_customers := COALESCE(NEW.ecommerce_auto_create_customers, false);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'marketplace_orders_business_partner_id_fkey'
  ) THEN
    ALTER TABLE public.marketplace_orders
      ADD CONSTRAINT marketplace_orders_business_partner_id_fkey
      FOREIGN KEY (business_partner_id) REFERENCES crm.business_partners(id) ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'marketplace_orders_customer_id_fkey'
  ) THEN
    ALTER TABLE public.marketplace_orders
      ADD CONSTRAINT marketplace_orders_customer_id_fkey
      FOREIGN KEY (customer_id) REFERENCES crm.customers(id) ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'marketplace_orders_sales_order_id_fkey'
  ) THEN
    ALTER TABLE public.marketplace_orders
      ADD CONSTRAINT marketplace_orders_sales_order_id_fkey
      FOREIGN KEY (sales_order_id) REFERENCES crm.sales_orders(id) ON DELETE SET NULL;
  END IF;
END;
$$;

CREATE UNIQUE INDEX IF NOT EXISTS uq_marketplace_orders_sales_order_id
  ON public.marketplace_orders (sales_order_id)
...
SET is_ecommerce = COALESCE(is_ecommerce, false);

ALTER TABLE crm.business_partners
  ALTER COLUMN is_ecommerce SET DEFAULT false;

ALTER TABLE crm.business_partners
  ALTER COLUMN is_ecommerce SET NOT NULL;
...
ALTER TABLE crm.sales_orders
  ...
  ADD COLUMN IF NOT EXISTS marketplace_order_id uuid NULL;

UPDATE crm.sales_orders
SET source_channel = COALESCE(NULLIF(lower(trim(source_channel)), ''), 'manual');
...
ALTER TABLE crm.sales_orders
  DROP CONSTRAINT IF EXISTS crm_sales_orders_source_channel_check;

ALTER TABLE crm.sales_orders
  ADD CONSTRAINT crm_sales_orders_source_channel_check
  CHECK (source_channel IN ('manual', 'marketplace'));
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'crm_sales_orders_marketplace_order_id_fkey'
  ) THEN
    ALTER TABLE crm.sales_orders
      ADD CONSTRAINT crm_sales_orders_marketplace_order_id_fkey
      FOREIGN KEY (marketplace_order_id) REFERENCES public.marketplace_orders(id) ON DELETE SET NULL;
  END IF;
END;
$$;

CREATE UNIQUE INDEX IF NOT EXISTS uq_crm_sales_orders_marketplace_order_id
  ON crm.sales_orders (marketplace_order_id)
...
CREATE OR REPLACE FUNCTION crm.assign_sales_order_number()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public, crm
AS $function$
DECLARE
  v_year integer := EXTRACT(YEAR FROM COALESCE(NEW.created_at, timezone('utc', now())))::integer;
  v_next_sequence bigint := 0;
BEGIN
  IF NEW.workspace_id IS NULL THEN
    RAISE EXCEPTION 'Sales orders require a workspace';
  END IF;
  ...
  PERFORM pg_advisory_xact_lock(hashtextextended(format('crm.sales_orders:%s:%s', NEW.workspace_id, v_year), 0));
  ...
  NEW.order_number := format('SO-%s-%s', v_year, lpad(v_next_sequence::text, 5, '0'));
  RETURN NEW;
END;
$function$;
...
FOR EACH ROW
EXECUTE FUNCTION crm.assign_sales_order_number();
...
DECLARE
  ...
  v_is_guest_profile boolean := false;
...
BEGIN
  ...
  IF v_sales_order_id IS NULL THEN
    ...
    IF COALESCE(v_workspace.ecommerce_auto_create_customers, false) THEN
      SELECT
        bp.id,
    ...
    ELSE
      ...
      WHERE bp.workspace_id = v_order.workspace_id
        AND COALESCE(bp.is_deleted, false) = false
        AND bp.merged_into_business_partner_id IS NULL
        AND COALESCE(bp.is_ecommerce, false) = true
        AND bp.name = 'E-Commerce Guest'
        AND COALESCE(NULLIF(trim(COALESCE(bp.phone, '')), ''), '') = ''
        AND COALESCE(NULLIF(trim(COALESCE(bp.email, '')), ''), '') = ''
      ...
      IF v_business_partner_id IS NULL THEN
        ...
        VALUES (
          ...
          NULL,
          NULL,
          'Shared guest profile for delivered marketplace orders when dedicated customer creation is disabled.',
          lower(COALESCE(v_order.currency, v_workspace.default_currency::text, 'usd')),
          'customer',
          0,
          NULL,
    ...
    END IF;
    IF v_is_guest_profile THEN
      v_order_notes := format('%s%sDelivered using the shared E-Commerce guest profile.', v_order_notes, E'\n\n');
    END IF;
    ...
    VALUES (
      ...
      CASE
        WHEN v_is_guest_profile THEN 'Marketplace receivable registered using the shared E-Commerce guest profile.'
        ELSE 'Marketplace receivable registered on delivery.'
      END,
      auth.uid(),
      NULL,
      jsonb_build_object(
        ...
        'receivable', true,
        'marketplaceOrderId', v_order.id,
        'marketplaceOrderNumber', v_order.order_number,
        'customerProfileMode', CASE WHEN v_is_guest_profile THEN 'guest' ELSE 'dedicated' END
      ),
      v_delivery_timestamp,
      v_delivery_timestamp,
      1,
  ...
  RETURN jsonb_build_object(
    ...
    'profile_mode', CASE WHEN v_is_guest_profile THEN 'guest' ELSE 'dedicated' END,
    'warning',
      CASE
