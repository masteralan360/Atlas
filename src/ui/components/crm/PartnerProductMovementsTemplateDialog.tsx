...
import {
  ...
  isPartnerProductMovementsColumnVisible,
  type PartnerProductMovementsColumnId,
...
type PartnerProductMovementsTemplateDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  templates: PartnerProductMovementsTemplate[]
  activeTemplate: PartnerProductMovementsTemplate
...
function cloneConfiguration(configuration: PartnerProductMovementsTemplateConfiguration) {
  return createPartnerProductMovementsTemplateConfiguration({
    ...configuration,
...
function getColumnLabel(
  ...
  const labels: Record<PartnerProductMovementsColumnId, [string, string]> = {
...
function isSameConfiguration(
  left: PartnerProductMovementsTemplateConfiguration,
  right: PartnerProductMovementsTemplateConfiguration
) {
  return JSON.stringify(left) === JSON.stringify(right)
}

export function PartnerProductMovementsTemplateDialog({
  open,
  onOpenChange,
  templates,
  activeTemplate,
  ...
  onSetDefault,
  onDeleteTemplate
}: PartnerProductMovementsTemplateDialogProps) {
  const { t } = useTranslation()
  const { toast } = useToast()
  const [draftTemplateId, setDraftTemplateId] = useState<string | null>(null)
  const [draftLabel, setDraftLabel] = useState('')
  const [draftConfiguration, setDraftConfiguration] = useState<PartnerProductMovementsTemplateConfiguration>(
    DEFAULT_PARTNER_PRODUCT_MOVEMENTS_TEMPLATE_CONFIGURATION
  )
  const [isProcessing, setIsProcessing] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<PartnerProductMovementsTemplate | null>(null)

  const resetDraft = (template: PartnerProductMovementsTemplate, draftId: string | null = template.id) => {
  ...
  const visibleColumnCount = PARTNER_PRODUCT_MOVEMENTS_COLUMN_IDS.filter((columnId) =>
    isPartnerProductMovementsColumnVisible(draftConfiguration, columnId)
  ).length
  ...
  const createNewTemplate = (copyFromActive: boolean) => {
    ...
    setDraftConfiguration(cloneConfiguration(
      copyFromActive ? activeTemplate.configuration : DEFAULT_PARTNER_PRODUCT_MOVEMENTS_TEMPLATE_CONFIGURATION
    ))
  }

  const toggleColumnVisibility = (columnId: PartnerProductMovementsColumnId, visible: boolean) => {
  ...
  const moveColumn = (columnId: PartnerProductMovementsColumnId, direction: -1 | 1) => {
    setDraftConfiguration((current) => {
  ...
  const setDefault = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: t('businessPartners.productMovements.defaultTemplateSavedDescription')
      })
    } catch {
      toast({
        title: t('businessPartners.accountStatement.templateSaveFailedTitle', { defaultValue: 'Could not save template' }),
          ...
          <AppDialogHeader>
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <AppDialogTitle>{t('businessPartners.productMovements.templatesTitle')}</AppDialogTitle>
            ...
            <section className="rounded-2xl border bg-muted/20 p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div className="min-w-0 flex-1 space-y-2">
                  <label htmlFor="partner-product-movements-template" className="text-sm font-semibold">
                  ...
                  >
                    <SelectTrigger id="partner-product-movements-template" className="h-11">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {!hasStoredTemplates ? (
              ...
              <>
                ...
                <section className="space-y-3 rounded-2xl border p-4">
                  <div className="space-y-2">
                    <label htmlFor="partner-product-movements-template-name" className="text-sm font-semibold">
                      {t('businessPartners.accountStatement.templateName', { defaultValue: 'Template name' })} *
                    </label>
                    <Input
                      id="partner-product-movements-template-name"
                      required
                      value={draftLabel}
                ...
                <section className="space-y-3 rounded-2xl border p-4">
                  ...
                  <div className="flex items-start justify-between gap-3 rounded-xl bg-muted/30 p-3">
                    <div className="space-y-1">
                      <label htmlFor="partner-movements-accumulate" className="text-sm font-medium">{t('businessPartners.productMovements.accumulate')}</label>
                      <p className="text-xs text-muted-foreground">{t('businessPartners.productMovements.accumulateDescription')}</p>
                    </div>
                    <Switch id="partner-movements-accumulate" checked={draftConfiguration.accumulateProducts}
                      onCheckedChange={(accumulateProducts) => setDraftConfiguration(current => ({ ...current, accumulateProducts }))} disabled={isProcessing} />
                  </div>
                </section>
                ...
                <section className="space-y-3 rounded-2xl border p-4">
                  ...
                  <div className="divide-y rounded-xl border bg-muted/10">
                    {draftConfiguration.columnOrder.map((columnId, index) => {
                      const isVisible = isPartnerProductMovementsColumnVisible(draftConfiguration, columnId)
