...
async function syncUpsertTransactions(
    ...
    try {
        ...
        console.error('[InventoryTransactions] Failed to sync transactions:', error)
        await queueOfflineUpserts(transactions, workspaceId)
    }
}
        ...
        async function fetchFromSupabase() {
            ...
            await db.transaction('rw', db.inventory_transactions, async () => {
                for (const remoteItem of data) {
                    const localItem = toCamelCase(remoteItem as Record<string, unknown>) as unknown as InventoryTransaction
